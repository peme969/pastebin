...
import runJs from '../public/run.js';
import apiDocsMd from '../public/api-docs.md';
    ...
    async fetch(request, env, ctx) {
        ...
        if (path === "/run.js") {
            return new Response(runJs, {
                headers: { 'Content-Type': 'application/javascript', ...getCORSHeaders() },
            });
        }
        ...
        if (path.startsWith("/api/")) {
            ...
            if (path.startsWith("/api/create") && method === "POST") {
                ...
                const expirationDate = expiration ? parseHumanReadableDate(expiration) : null;
                
                if (expiration && !expirationDate) {
                    return new Response("Invalid expiration format. Use 'YYYY-MM-DD hh:mm AM/PM'.", { 
                        status: 400,
                ...
                const metadata = {
                    password,
                    expirationInSeconds: expirationDate ? getSecondsRemaining(expirationDate.getTime()) : null,
                    formattedExpiration: expirationDate ? formatTimestamp(expirationDate.getTime()) : null,
            ...
            if (path.startsWith("/api/delete") && method === "DELETE") {
                ...
                if (!slug) return new Response("Missing slug", { status: 400, headers: getCORSHeaders() });
            ...
            if (path.startsWith("/api/view") && method === "GET") {
                ...
                if (slug === "view") {
                    ...
                    if (!paste) return new Response("Paste not found", { 
                        status: 404, 
                        headers: { "Content-Type": "application/json", ...getCORSHeaders() } 
                    });
        ...
        if (slug && !slug.includes('/')) { // Make sure it's just a slug, not a path
            const paste = await PASTEBIN_KV.get(slug, { type: "json" });
